	...
	//myAccount.jspで変更ボタンを押した時の処理
			...
			if (session != null) {
				...
				if (user != null) {
					...
					RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/user/changePlofile.jsp");
		...
		// ① フォーム送信時
			...
			String confirmPass = request.getParameter("confirmPass");
				...
				if (name == null || name.isEmpty() ||
					...
					request.getRequestDispatcher("/WEB-INF/jsp/user/changePlofile.jsp").forward(request, response);
					return;
				}
				...
				//確認画面へフォワード
				RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/user/changePlofileCheck.jsp");
				dispatcher.forward(request, response);
			}	
			...
			//確認画面のボタンの分岐	
				...
				if ("戻る".equals(step)) {
					...
					//フォワード
					RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/user/changeUserInfo.jsp");
					dispatcher.forward(request, response);
				}
					...
					try {
						...
						if (success) {//変更がうまくいけばTrue
							...
							RequestDispatcher dispatcher = request
									.getRequestDispatcher("/WEB-INF/jsp/user/changePlofileResult.jsp");
							dispatcher.forward(request, response);
						}
